<template>
  ...
  >
    ...
    <Loading
      v-if="tab_item == 'followees'"
      :empty="followees_data == null"
      :loading="followees_loading"
      :pagination="followees_pagination"
      @autoload="GetUsersFollowees"
    />
    <Loading
      v-if="tab_item == 'followers'"
      :empty="followers_data == null"
      :loading="followers_loading"
      :pagination="followers_pagination"
      @autoload="GetUsersFollowers"
    />
    <Loading
...
import { GetFollows, GetUsers, Get_G_USERS_RECOMMENDED } from '@/api/global.js'
    ...
    async GetUsersFollowees() {
      ...
      if (response.data.is_get) {
        ...
        this.followees_data == null
          ? (this.followees_data = response.data.data)
          : this.$G_FilterSameItems(keys, this.followees_data, response.data.data)
    ...
    async GetUsersFollowers() {
      ...
      if (response.data.is_get) {
        ...
        this.followers_data == null
          ? (this.followers_data = response.data.data)
          : this.$G_FilterSameItems(keys, this.followers_data, response.data.data)
...
// @import '../../../views/client/topics/index.less';
</style>
