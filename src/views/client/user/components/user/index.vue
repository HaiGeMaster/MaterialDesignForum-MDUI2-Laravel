<template>
  ...
  >
    ...
    <div
      ...
      class="cover"
      ref="cover"
      :style="`background-image: url(${$G_ImgHandle(user.cover.large)})`"
    >
      <div class="mc-cover-upload" v-if="$G_GetUserID() == user.user_id">
        <mdui-button-icon
          variant="tonal"
          @click="handleFileSelectCover()"
      ...
      <div style="padding: 16px">
        ...
        <mdui-chip>
          <mdi-icon
            slot="icon"
            :styles="`color:${user.user_group.user_group_color};`"
    ...
    <div v-if="user" class="info">
      <div class="avatar-box">
        <div class="mc-avatar-upload" v-if="$G_GetUserID() == user.user_id">
          <mdui-button-icon
            variant="tonal"
            class="upload-btn glass-container"
            @click="handleFileSelectAvatar()"
      ...
      >
        ...
        <div class="meta" v-if="user.headline">
          <mdui-tooltip
            :content="$t('Message.Client.User.User.OneSentenceIntroduction')"
            placement="left"
          >
        ...
        <div class="meta" v-if="user.company">
          <mdui-tooltip
            :content="$t('Message.Client.User.User.AffiliatedSchoolOrEnterprise')"
            placement="left"
          >
        ...
        <div class="meta" v-if="user.bio">
          <mdui-tooltip
            :content="$t('Message.Client.User.User.PersonalIntroduction')"
            placement="left"
          >
        ...
        <mdui-button
          class="fold-button"
          variant="elevated"
          v-if="ComputedFoldButtonRule"
          @click="profile_fold = !profile_fold"
        >
          <mdi-icon slot="icon" :icon="profile_fold ? 'mdi-chevron-down' : 'mdi-chevron-up'" />
          {{
            profile_fold
              ? $t('Message.Client.User.User.ShowDetail')
              : $t('Message.Client.User.User.CollapseDetails')
          }}
        </mdui-button>
      </div>
    </div>
    ...
    <div v-if="user" class="actions">
      <EditInfoButton
        v-if="userStore.getIsLogin && userStore.getUser.user_group['ability_edit_own_info']"
        :user="user"
      />
      <FollowButton
...
import { UploadAvatar, UploadCover } from '@/api/global.js'
