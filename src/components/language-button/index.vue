<template>
  ...
  <mdui-dropdown>
    ...
    <mdui-menu-item v-else-if="slotname == 'list'" slot="trigger">
      <mdi-icon icon="mdi-translate" slot="icon" />
      {{ $t('Message.langInfo.langname') }}
      <mdi-icon icon="mdi-dots-vertical" slot="end-icon" />
    </mdui-menu-item>
    ...
    <mdui-menu selects="single">
      ...
      >
        ...
        <span
          ...
          slot="icon"
          style="width: 26px"
          :class="`langicon fi fi-${item.flagicon}`"
        ></span>
        {{ item.text }}
...
export default {
  ...
  methods: {
    ...
    async SetUserLanguage(val) {
      // if (this.userStore.getIsLogin == false) {
