<template>
  <mdui-dialog close-on-overlay-click @close="vmodel = false" style="margin: auto;" ref="report_dialog"
      ...
      <!-- <mdui-card > -->
        <mdui-radio-group :value="radio_group">
          <template v-for="n in radio_group_item">
            <br>
            <mdui-radio :value="n.value" @click="radio_group = n.value">{{ $t(n.text) }}</mdui-radio>
          </template>
        </mdui-radio-group>
        <mdui-text-field :value="other_reasons" @input="other_reasons = $event.target.value" style="padding: 8px;"
          v-if="radio_group == 'Message.Components.OptionsButton.OtherReasons'"
          :label="$t('Message.Components.OptionsButton.OtherReasons')"></mdui-text-field>
      </mdui-card>
    </div>
