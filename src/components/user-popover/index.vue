<template>
  <!-- :trigger="!mainStore.getMobile?'hover':'click'" 由于mdui组件不能正确检测其下拉框里面的内容导致有bug -->
  <mdui-dropdown class="mc-user-popover" v-if="user">
    <div slot="trigger" @click.stop="null">
      <mdui-avatar :src="$G_ImgHandle(user.avatar.middle || '')"> </mdui-avatar>
      <mdui-badge
      ...
      <mdui-card variant="outlined" style="width: 300px" class="mdui-card-variant-color-border">
        ...
        >
          ...
          <div slot="icon" @click.stop="null">
            <mdui-avatar :src="$G_ImgHandle(user.avatar.small || '')"> </mdui-avatar>
        ...
        <div class="stats">
          <mdui-segmented-button-group :full-width="true" style="width: 100%">
            <mdui-segmented-button
              @click.prevent.stop="
                $router.push(`${$G_UrlHeaderLang()}/users/${user.user_id}#answers`)
              "
              >{{ $t('Message.Components.UserPopover.Answer') }}
              {{ user.answer_count }}</mdui-segmented-button
            >
            <mdui-segmented-button
              @click.prevent.stop="
                $router.push(`${$G_UrlHeaderLang()}/users/${user.user_id}#articles`)
              "
              >{{ $t('Message.Components.UserPopover.Article') }}
              {{ user.article_count }}</mdui-segmented-button
            >
            <mdui-segmented-button
              @click.prevent.stop="
              ...
              {{ user.follower_count }}</mdui-segmented-button
            >
