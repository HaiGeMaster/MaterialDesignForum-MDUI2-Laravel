<template>
  ...
  >
    ...
    >
      <mdi-icon slot="icon" :icon="show_replies ? 'mdi-menu-up' : 'mdi-menu-down'" />
...
import { GetReplys } from '@/api/global.js'
export default {
  name: 'comments-item',
  props: {
    classes: {
  ...
  methods: {
    ...
    async GetReplys() {
      ...
      if (response.data.is_get == true) {
        ...
        this.data == null
          ? (this.data = response.data.data)
          : this.$G_FilterSameItems('reply_id', this.data, response.data.data)
