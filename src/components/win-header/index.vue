<template>
    ...
    :style="{
        'background-color': $store.getters.GetScrollValue==0?'rgb(var(--mdui-color-surface))':'rgb(var(--mdui-color-surface-container))'
    }"
    >
        ...
        <div class="flex-grow" v-if="$store.getters.GetPc" style="-webkit-app-region: drag;-webkit-user-select: none;"/>

        <div class="window-name" style="-webkit-app-region: drag;-webkit-user-select: none;">{{ title }}</div>
        ...
        <div class="window-controls">
            ...
            <mdui-tooltip :content="!top_window ? $t('Message.Components.SystemBar.TopWindow') :
                    $t('Message.Components.SystemBar.CancelTopWindow')">

                <mdui-button-icon @click="top_window = !top_window"
                style="height: 16px;width: 16px;margin: 0 4px 0 4px;"
                >
                    <mdi-icon :icon="!top_window ? 'mdi-brightness-1' : 'mdi-radiobox-marked'"
            ...
            <mdui-tooltip :content="$t('Message.Components.Editor.Minimize')">
                <mdui-button-icon @click="_OnWindowOpen('window-min')"
                style="height: 16px;width: 16px;margin: 0 4px 0 4px;"
                >
                    <mdi-icon icon="mdi-brightness-1"
                    span_style="color: #64af4b;"
                    ></mdi-icon>
                </mdui-button-icon>
            </mdui-tooltip>

            <mdui-tooltip :content="$t('Message.Components.Editor.Maximize')">
                <mdui-button-icon @click="_OnWindowOpen('window-max')"
                style="height: 16px;width: 16px;margin: 0 4px 0 4px;"
                >
                    <mdi-icon icon="mdi-brightness-1"
                    span_style="color: #f09800;"
                    ></mdi-icon>
                </mdui-button-icon>
            </mdui-tooltip>

            <mdui-tooltip :content="$t('Message.Components.Editor.Close')">
                <mdui-button-icon @click="_OnWindowOpen('window-close')"
                style="height: 16px;width: 16px;margin: 0 4px 0 4px;"
                >
                    <mdi-icon icon="mdi-brightness-1"
                    span_style="color: #e1423b;"
                    ></mdi-icon>
                </mdui-button-icon>
            </mdui-tooltip>
